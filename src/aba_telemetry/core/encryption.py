"""Encryption at rest for aba-telemetry backends.
...
Or use the module-level convenience functions::
    ...
    from aba_telemetry.core.encryption import encrypt_line, decrypt_line
    ct = encrypt_line(key, "some text")
    pt = decrypt_line(key, ct)

Requires: pip install aba-telemetry[encryption]
"""
...
_SALT_PEPPER = b"aba-telemetry-v1:"
...
def get_encryption_key() -> Optional[bytes]:
    ...
    Priority:
        ...
        2. OS keyring: keyring.get_password("aba-telemetry", "encryption-key")
    ...
    if not _CRYPTO_AVAILABLE:
        logger.debug(
            ...
            "Install: pip install aba-telemetry[encryption]"
        )
        return None
    ...
    try:
        ...
        stored = keyring.get_password("aba-telemetry", "encryption-key")
        if stored:
