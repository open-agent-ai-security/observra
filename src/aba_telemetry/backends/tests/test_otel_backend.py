...
class TestGenAIAttributes:
    ...
    def test_write_sets_custom_namespace_attributes(self, captured_spans):
        """Custom aba_telemetry.* namespace attributes are always set."""
        backend = OTelExportBackend()
        event = _model_event()
        backend.write(event)
        ...
        assert "aba_telemetry.event_id" in attrs
        assert "aba_telemetry.event_type" in attrs
        assert "aba_telemetry.trace_id" in attrs
        assert "aba_telemetry.session_id" in attrs
        assert "aba_telemetry.framework" in attrs
    ...
    def test_write_cost_usd_attribute(self, captured_spans):
        """cost_usd in event.data is stored as aba_telemetry.cost_usd."""
        backend = OTelExportBackend()
        ...
        assert len(captured_spans) == 1
        attrs = captured_spans[0].attributes
        assert abs(attrs["aba_telemetry.cost_usd"] - 0.005) < 1e-9
...
class TestInjectionAttributes:
    ...
    def test_injection_detected_attributes(self, captured_spans):
        ...
        assert attrs["aba_telemetry.injection_patterns"] == "JAILBREAK_DAN,BASE64_BLOB"
...
class TestQueryAndErrors:
    ...
    def test_write_error_increments_error_count(self, captured_spans, monkeypatch):
        ...
        import aba_telemetry.backends.otel as otel_module
