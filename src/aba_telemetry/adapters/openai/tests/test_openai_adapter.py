...
def _patch_span_classes(monkeypatch):
    ...
    from aba_telemetry.adapters.openai import adapter as adapter_mod
...
def _make_function_span(name="calculator", input_val="2+3", output_val="5",
                        span_id="span-func-001"):
...
def _make_handoff_span(from_agent="orchestrator", to_agent="specialist",
                       error=None, parent_id="span-func-001"):
...
def test_openai_adapter_satisfies_protocol():
    ...
    assert isinstance(adapter, FrameworkAdapter), (
        "OpenAIAdapter does not satisfy FrameworkAdapter Protocol"
    )
...
def test_function_span_captures_tool_data_when_enabled():
    ...
    adapter.on_span_end(
        _make_function_span(name="lookup", input_val="pi", output_val="3.14159")
    )
...
def test_handoff_span_emits_handoff_event():
    ...
    adapter = OpenAIAdapter()
    adapter.on_span_end(
        _make_handoff_span(from_agent="orchestrator", to_agent="specialist")
    )
...
def test_cost_threshold_exceeded_emitted_and_only_once():
    ...
    event_types = [e.event_type for e in adapter._events]
    assert "cost_threshold_exceeded" in event_types, (
        f"Expected cost_threshold_exceeded event; got: {event_types}"
    )
...
def test_normalize_openai_tokens():
    ...
    from aba_telemetry.adapters.utils import normalize_openai_tokens, NormalizedTokens
