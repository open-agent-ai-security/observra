...
class TestReleasingMdSections:
    def test_pre_tag_checklist_section_exists(self):
        ...
        assert "## Pre-Tag Checklist" in _content(), "docs/RELEASING.md missing '## Pre-Tag Checklist' section"
    ...
    def test_tag_push_flow_section_exists(self):
        ...
        assert "## Tag Push Flow" in _content(), "docs/RELEASING.md missing '## Tag Push Flow' section"
    ...
    def test_rollback_procedure_section_exists(self):
        ...
        assert "## Rollback Procedure" in _content(), "docs/RELEASING.md missing '## Rollback Procedure' section"

    def test_version_history_note_section_exists(self):
        """docs/RELEASING.md must contain a 'Version History Note' H2 section."""
        assert "## Version History Note" in _content(), "docs/RELEASING.md missing '## Version History Note' section"
...
class TestReleasingMdRollbackContent:
    def test_rollback_contains_npm_unpublish(self):
        ...
        assert "npm unpublish" in _content(), "docs/RELEASING.md Rollback section missing 'npm unpublish'"

    def test_rollback_contains_twine_yank(self):
        """Rollback section must document 'twine yank' command."""
        assert "twine yank" in _content(), "docs/RELEASING.md Rollback section missing 'twine yank'"

    def test_rollback_contains_gh_release_delete(self):
        """Rollback section must document 'gh release delete' command."""
        assert "gh release delete" in _content(), "docs/RELEASING.md Rollback section missing 'gh release delete'"

    def test_rollback_contains_git_tag_delete(self):
        """Rollback section must document 'git tag -d' command."""
        assert "git tag -d" in _content(), "docs/RELEASING.md Rollback section missing 'git tag -d'"
...
class TestReleasingMdPep440Warning:
    def test_version_history_note_warns_about_pep440_shadowing(self):
        ...
        assert has_pep440 or has_prerelease_warning, (
            "docs/RELEASING.md Version History Note does not warn about PEP 440 pre-release version shadowing"
        )
