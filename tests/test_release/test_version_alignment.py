...
class TestVersionAlignment:
    def test_pyproject_toml_version_matches_cargo_toml(self):
        ...
        assert py_ver == cargo_ver, f"pyproject.toml version {py_ver!r} != rust/Cargo.toml version {cargo_ver!r}"
    ...
    def test_pyproject_toml_version_matches_npm_installer(self):
        ...
        assert py_ver == npm_ver, f"pyproject.toml version {py_ver!r} != npm installer version {npm_ver!r}"
    ...
    def test_fwd_darwin_arm64_version_matches_pyproject(self):
        ...
        assert py_ver == pkg_ver, f"pyproject.toml {py_ver!r} != npm/fwd-darwin-arm64 {pkg_ver!r}"
    ...
    def test_fwd_darwin_x64_version_matches_pyproject(self):
        ...
        assert py_ver == pkg_ver, f"pyproject.toml {py_ver!r} != npm/fwd-darwin-x64 {pkg_ver!r}"
    ...
    def test_fwd_linux_arm64_version_matches_pyproject(self):
        ...
        assert py_ver == pkg_ver, f"pyproject.toml {py_ver!r} != npm/fwd-linux-arm64 {pkg_ver!r}"
    ...
    def test_fwd_linux_x64_version_matches_pyproject(self):
        ...
        assert py_ver == pkg_ver, f"pyproject.toml {py_ver!r} != npm/fwd-linux-x64 {pkg_ver!r}"
    ...
    def test_fwd_win32_x64_version_matches_pyproject(self):
        ...
        assert py_ver == pkg_ver, f"pyproject.toml {py_ver!r} != npm/fwd-win32-x64 {pkg_ver!r}"
    ...
    def test_all_six_npm_packages_have_identical_versions(self):
        ...
        all_packages = [NPM_INSTALLER] + [REPO_ROOT / "npm" / pkg / "package.json" for pkg in PLATFORM_PACKAGES]
        versions = {str(p.parent.name): _npm_version(p) for p in all_packages}
        unique_versions = set(versions.values())
        assert len(unique_versions) == 1, f"npm package versions are not all equal: {versions}"
    ...
    def test_version_string_is_semver_format(self):
        ...
        assert re.fullmatch(r"\d+\.\d+\.\d+", ver), f"Version {ver!r} is not a clean X.Y.Z semver string"
