...
def test_create_event_validates_empty_type():
    ...
    with pytest.raises(ValueError, match="event_type cannot be empty"):
        TelemetryEvent(
            event_id="test", timestamp=1234.5, trace_id="trace", session_id="session", span_id="span", event_type=""
        )
...
def test_create_event_hot_path_strips_strings():
    ...
    event = create_event(
        "model_request", input_tokens=100, output_tokens=50, model="gemini-2.5-flash", user_content="some text"
    )
...
def test_create_event_cold_path_redacts():
    ...
    event = create_event("model_error", error_message="Failed with api_key=sk_test_abc123def456ghi")
